...
impl ShapeProcessor {
    pub fn extract_planet_data(&self, tile_processor: &mut TileProcessor) {
        ...
        Self::extract_water_shapes(&thread_pool, tx.clone(), self.world_boundary);
    ...
    fn extract_water_shapes(
        thread_pool: &ThreadPool,
        sender: Sender<(MapGeomObject, MapGeometry)>,
        world_boundary: Rect,
    ) {
        println!("Extract water shapes");
        thread_pool.execute(move || {
            ...
            print!("\rWater shapes extracted, count: {}\n", shapes_amount);
        });
    }
